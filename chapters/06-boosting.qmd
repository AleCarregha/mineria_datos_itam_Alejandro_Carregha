...
ada = AdaBoostClassifier(
    ...
    algorithm='SAMME.R',                             # SAMME.R usa probabilidades
    random_state=42
)
...
4. **`algorithm`**:
   - `'SAMME'`: Versión discreta (clasificaciones duras)
   - `'SAMME.R'`: Versión basada en probabilidades (usualmente mejor)
...
Finalmente, veamos un ejemplo de hyperparameter tuning con Grid Search:

```{python}
#| label: xgboost-tuning
#| fig-cap: "Grid Search para hiperparámetros de XGBoost"
#| warning: false

from sklearn.model_selection import GridSearchCV

# Definir grid de hiperparámetros (versión simplificada)
param_grid = {
    'max_depth': [3, 5, 7],
...
# Usar subset de datos para velocidad
...
print(f"\nMejor score (RMSE): {-grid_search.best_score_:.4f}")
...
scores = -top_10['mean_test_score']
stds = top_10['std_test_score']

ax.bar(x_pos, scores, yerr=stds, capsize=5, color='#9b59b6', alpha=0.7)
ax.set_xticks(x_pos)
ax.set_xticklabels(
    [f"Config {i+1}" for i in range(len(top_10))],
    rotation=45
)
ax.set_ylabel('RMSE')
ax.set_title('Top 10 Configuraciones de Hiperparámetros')
...
- Necesitas el mejor balance entre precisión y velocidad
- Quieres una implementación madura y bien documentada
- Participas en competencias de ML (es el estándar de facto)
- Necesitas flexibilidad en funciones objetivo y métricas de evaluación
:::

:::{.callout-important}
## Instalación de XGBoost

Para usar XGBoost, instálalo con:
```bash
conda install xgboost
```

Para soporte de GPU (opcional):
```bash
...
Para soporte de GPU:
```bash
conda install lightgbm --install-option=--gpu
```
:::
...
**Elige sklearn GradientBoosting si:**
- 📖 **Aprendizaje**: Estás aprendiendo los conceptos fundamentales
- 🔬 **Datasets pequeños**: Tienes <10,000 muestras
- 🧩 **Simplicidad**: Quieres la API más simple y familiar
- 🔗 **Integración**: Ya usas sklearn extensivamente
- 🎓 **Enseñanza**: Estás enseñando conceptos de ML
...
**Métodos previos:**
- **Regresión lineal/logística** (Capítulo 3): Boosting puede usar estos como base learners, aunque árboles son más comunes
- **Árboles de decisión** (Capítulo 5): Los árboles individuales son los weak learners típicos en boosting
- **Random Forest** (Capítulo 5): Hermano "paralelo" de boosting; ambos son ensemble methods

**Métodos futuros:**
- **Redes neuronales**: Complementarias a boosting; boosting domina en tabular, redes en imágenes/texto/audio
- **Stacking/Blending**: Pueden combinar múltiples modelos de boosting para mejoras marginales
- **AutoML**: Los sistemas de AutoML típicamente prueban múltiples algoritmos de boosting con diferentes hiperparámetros
